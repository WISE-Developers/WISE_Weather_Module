...
#define CWFGM_WINDSPEED_GRID_VERSION	11
#ifndef DOXYGEN_IGNORE_CODE
struct speed_entry
{
	speed_entry() { m_data = nullptr; m_datavalid = nullptr; m_speed = -1.0; }
	speed_entry(const speed_entry &toCopy, std::uint16_t xsize, std::uint16_t ysize);
	speed_entry(double speed, const std::string &fn, std::uint16_t *d, bool *dv) : filename(fn) { m_speed = speed; m_data = d; m_datavalid = dv; }
	std::string filename;
	double m_speed;
	std::uint16_t *m_data;
	bool *m_datavalid;
...
class SpeedSector : public Sector
{
	public:
		SpeedSector(double mn, double mx, const std::string &l) : Sector(mn, mx, l) { }
		SpeedSector(const SpeedSector &toCopy, std::uint16_t xsize, std::uint16_t ysize);
		~SpeedSector()
		{
			m_entries.clear();
		}
		void Cleanup()
		{
			for (std::uint16_t i=0; i<m_entries.size(); i++)
			{
				...
				if (m_entries[i].m_datavalid)	delete [] m_entries[i].m_datavalid;
			}
		}
		...
		{
			speed_entry entry(speed, filename, d, v);
		...
		SpeedSector* ShallowCopy()
		{
			SpeedSector* ss = new SpeedSector(m_minAngle, m_maxAngle, m_label);
			for (std::uint16_t i = 0; i < m_entries.size(); i++)
			{
		...
		std::vector<speed_entry> m_entries;
};
#endif
...
/**
	...
	In addition to the GridEngine interface, it also implements its own interface so that a client application may specify a collection of wind speed grids for use in a simulation.
	...
	wind speed grid information loaded into it, to be applied when specific input wind speeds are encountered.  This object is unconcerned about
	how other weather data has been provided: if times and other input data are valid, then this object will apply the updated wind speed data according to the
...
public:
	...
	friend class CWFGM_WindSpeedGridHelper;
	CCWFGM_WindSpeedGrid();
	CCWFGM_WindSpeedGrid(const CCWFGM_WindSpeedGrid &toCopy);
	~CCWFGM_WindSpeedGrid();

#endif
	/**
		Creates a new wind speed grid with all the same properties and data of the object being called, returns a handle to the new object in 'newGrid'.
		No data is shared between these two objects, an exact copy (including of all loaded wind speed grid data) is created.
		\param	newGrid	A wind speed grid object.
		\sa ICWFGM_WindSpeedGrid::Clone
	...
	*/
	...
	/**
		Imports a Wind Speed Grid to a specific sector and assigned applicable speed ranges.  If the import projection file is unspecified, then it is assumed to match the
		...
		\sa ICWFGM_WindSpeedGrid::Import
		\retval	S_OK	Successful.
		...
		\retval	ERROR_SEVERITY_WARNING	Failed initialization or a non-specific I/O error.  Wind speed values are limited to 250kph.
	*/
	...
	/**
		...
		\param	prj_file_name	Projection File name
		\param	grid_file_name	Grid File name
		\sa ICWFGM_WindSpeedGrid::Export
		\retval	S_OK	Successful.
		\retval	ERROR_GRID_SIZE_INCORRECT	Incorrect grid size.
		\retval	E_POINTER			Error
		\retval	ERROR_GRID_UNINITIALIZED	Grid uninitialized.
	...
	*/
	...
	/**
		...
		\sa ICWFGM_WindSpeedGrid::SetAttribute
		\retval	S_OK	Successful.
	...
	*/
	...
	/**
		...
		\param	count	Stores the number of sectors for the filter
		\sa ICWFGM_WindSpeedGrid::GetSectorCount
		\retval	S_OK	Successful.
	*/
	...
	/**
		...
		\sa ICWFGM_WindSpeedGrid::GetSectorAngles
		\retval	S_OK	Successful.
		\retval	ERROR_SECTOR_INVALID_INDEX	Invalid Sector Index.
	*/
	...
	/**
		...
		\sa ICWFGM_WindSpeedGrid::AddSector
		\retval	S_OK	Successful.
	...
	*/
	...
	/**
		...
		\param	sector	Index of sector.
		\sa ICWFGM_WindSpeedGrid::RemoveSector
		\retval	S_OK
		\retval	ERROR_SECTOR_INVALID_INDEX	Invalid sector index.
	*/
	...
	/**
		...
		\sa ICWFGM_WindSpeedGrid::GetSector
		\retval	S_OK	Successful.
	...
	*/
	...
	/**
		...
		\retval ERROR_GRID_WEATHER_NOT_IMPLEMENTED	Invalid grid.
	*/
	virtual NO_THROW HRESULT Valid(Layer *layerThread, const HSS_Time::WTime &, const HSS_Time::WTimeSpan &duration, std::uint32_t option, std::vector<uint16_t> *application_count) override;
	/**
	...
	*/
	...
	protected:
		...
		std::vector<SpeedSector*>		m_sectors;
...
public:
	...
	virtual CCWFGM_WindSpeedGrid *deserialize(const google::protobuf::Message& proto, std::shared_ptr<validation::validation_object> valid, const std::string& name) override;
