/**
 * WISE_Weather_Module: CWFGM_WindDirectionGrid.Serialize.cpp
...
HRESULT CCWFGM_WindDirectionGrid::Import(const std::uint16_t sector, const double speed, const std::string & prj_file_name, const std::string & grid_file_name)
{
	HRESULT hr = S_OK;
	...
	std::uint16_t *WSArray = nullptr, *aa1;
	bool *NDArray = nullptr, *nd1;
	...
	for (i = 0, aa1 = WSArray, nd1 = NDArray; i < index; i++, aa1++, nd1++) {
		std::uint16_t ws;
		bool nd = true;
		double fws = importer.doubleData(1, i);
		if (fws == noData)
			nd = false;			// flag for no data
		else if (fws < 0.0 || fws > 360.0) {		// angle should be normalized to between 0 and 360 degrees
			delete [] WSArray;
			delete [] NDArray;
			return ERROR_SEVERITY_WARNING;
		}
		else
			ws = (std::int16_t)floor(fws + 0.5);
		if (nd)
			*aa1 = ws;
		*nd1 = nd;
	}
...
HRESULT CCWFGM_WindDirectionGrid::Export(const std::uint16_t sector, const double speed, const std::string & prj_file_name, const std::string & grid_file_name) {
	if (!grid_file_name.length())							return E_INVALIDARG;
	...
	GDALExporter::ExportResult res = exporter.Export(l_array, grid_file_name.c_str(), "Wind Dir.");
...
std::int32_t CCWFGM_WindDirectionGrid::serialVersionUid(const SerializeProtoOptions& options) const noexcept {
	return options.fileVersion();
}


WISE::WeatherProto::WindGrid* CCWFGM_WindDirectionGrid::serialize(const SerializeProtoOptions& options) {
...
CCWFGM_WindDirectionGrid* CCWFGM_WindDirectionGrid::deserialize(const google::protobuf::Message& proto, std::shared_ptr<validation::validation_object> valid, const std::string& name)
{
	...
	/// <summary>
	/// Child validations for a wind direction grid.
	/// </summary>
	...
	if (grid->has_startspan())
	{
		...
		if ((WTimeSpan(0, 0, 0, 0) > w) || (w > WTimeSpan(0, 23, 59, 59))) {
			if (myValid)
				/// <summary>
				/// The start time span for the wind direction grid is not in [00:00:00, 23:59:59].
				/// </summary>
				/// <type>user</type>
				myValid->add_child_validation("HSS.Times.WTimeSpan", "startSpan", validation::error_level::SEVERE,
					validation::id::time_range_invalid, w.ToString(WTIME_FORMAT_STRING_ISO8601),
					{ true, "PT0M" }, { true, "PT23H59M59S" });
			m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid start span value";
			throw ISerializeProto::DeserializeError("Invalid end span value");
		}
	...
	if (grid->has_endspan())
	{
		...
		if ((WTimeSpan(0, 0, 0, 0) > w) || (w > WTimeSpan(0, 23, 59, 59))) { 
			if (myValid)
				/// <summary>
				/// The end time span for the wind direction grid is not in [00:00:00, 23:59:59].
				/// </summary>
				/// <type>user</type>
				myValid->add_child_validation("HSS.Times.WTimeSpan", "endSpan", validation::error_level::SEVERE,
					validation::id::time_range_invalid, w.ToString(WTIME_FORMAT_STRING_ISO8601),
					{ true, "PT0M" }, { true, "PT23H59M59S" });
			m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid end span value";
			throw ISerializeProto::DeserializeError("Invalid end span value");
		}
		if (grid->has_startspan()) { 
			if (w < m_startSpan) {
				if (myValid)
					/// <summary>
					/// The end time span for the wind direction grid is earlier than the start time span.
					/// </summary>
					/// <type>user</type>
					myValid->add_child_validation("HSS.Times.WTimeSpan", { "startSpan", "endSpan" }, validation::error_level::SEVERE,
						validation::id::time_invalid, { m_startSpan.ToString(WTIME_FORMAT_STRING_ISO8601), w.ToString(WTIME_FORMAT_STRING_ISO8601) });
				m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid end span value";
				throw ISerializeProto::DeserializeError("Invalid end span value");
			}
		}
	...
	if (grid->has_defaultsectordata())
	{
		...
		if (defaults.has_binary())
		{
			...
			if (xsize != gridXDim) {
				m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid grid dimensions";
				if (defaultsValid)
					/// <summary>
			...
			if (defaults.binary().has_iszipped() && defaults.binary().iszipped().value())
			{
				...
				if (data.length() != (valid.length() * sizeof(std::uint16_t)) || valid.length() != sz) {
					if (defaultsValid)
						/// <summary>
					...
					m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid default wind direction grid in imported file.";
					throw ISerializeProto::DeserializeError("WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid default wind direction grid in imported file.");
				}
		...
		else if (defaults.has_file())
		{
			if (!defaults.file().has_projectionfilename())
			{
				if (defaultsValid)
					/// <summary>
					/// A file has been provided for the default sector data of a wind direction grid but no projection was provided with it.
					/// </summary>
					/// <type>user</type>
			...
			if ((m_ysize == (std::uint16_t)-1) && (m_xsize == (std::uint16_t)-1))
				gridEngine->GetDimensions(0, &m_xsize, &m_ysize);
			HRESULT hr1;
	...
	if (grid->sectordata_size() > 0)
	{
		...
		{
			/// <summary>
			/// Child validations for a wind direction grid sector.
			/// </summary>
			...
			if (!sector.has_direction()) {
				if (sectorValid)
					/// <summary>
					/// The wind direction sector data doesn't have a wind direction specified with it.
					/// </summary>
					/// <type>user</type>
			...
			if (sector.sectorentries_size() > 0)
			{
				...
				{
					/// <summary>
					/// Child validations for sector entries in a wind direction grid.
					/// </summary>
					...
					if (entry.version() == 1 && entry.has_speed() && entry.has_data() && entry.data().version() == 1)
					{
						...
						if (entry.data().has_binary())
						{
							...
							if (!s)
							{
								s = new NewDirectionSector(min_d, max_d, sector.label());
								add = true;
							}
							...
							if (xsize > 0 && ysize > 0)
							{
								...
								if (entry.data().binary().has_iszipped() && entry.data().binary().iszipped().value())
								{
									...
									if (arr.length() != (valid.length() * sizeof(std::uint16_t)) || valid.length() != sz)
									{
										...
										if (add)
											delete s;
										m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid wind direction grid in imported file.";
										throw ISerializeProto::DeserializeError("WISE.WeatherProto.CwfgmWindDirectionGrid: Invalid wind direction grid in imported file.");
									}
						...
						else if (entry.data().has_file())
						{
							if (!entry.data().file().has_projectionfilename())
							{
								if (dataValid)
									/// <summary>
									/// A file has been provided for the sector data of a wind direction grid but no projection was provided with it.
									/// </summary>
									/// <type>user</type>
							...
							if (FAILED(hr1 = Import(sector_i, speed, entry.data().file().projectionfilename().value(), entry.data().file().filename()))) {
								if (dataValid)
								{
									...
									case E_FAIL:
										/// <summary>
										/// The specified wind direction grid is not a valid file, it cannot be imported by GDAL.
										/// </summary>
										/// <type>user</type>
									...
									case ERROR_GRID_LOCATION_OUT_OF_RANGE:
										/// <summary>
										/// The projection of the specified wind direction grid does not match that of the elevation grid and fuelmap.
										/// </summary>
										/// <type>user</type>
										dataValid->add_child_validation("WISE.GridProto.wcsData.locationFile", "file", validation::error_level::SEVERE,
									...
									case ERROR_GRID_SIZE_INCORRECT:
										/// <summary>
										/// The specified wind direction grid is a different size than the elevation grid and fuelmap.
										/// </summary>
										/// <type>user</type>
										dataValid->add_child_validation("WISE.GridProto.wcsData.locationFile", "file", validation::error_level::SEVERE,
									...
									case ((HRESULT)ERROR_SEVERITY_WARNING):
										/// <summary>
										/// A wind direction in the wind direction grid is not valid. Wind directions must be in [0, 360].
										/// </summary>
										/// <type>user</type>
										dataValid->add_child_validation("WISE.GridProto.wcsData.locationFile", "file", validation::error_level::SEVERE,
											validation::id::wind_grid_direction, entry.data().file().filename(),
											{ true, 0.0 }, { true, 360.0 });
										break;
									}
								}
								m_loadWarning = "Error: WISE.WeatherProto.CwfgmWindDirectionGrid: Wind direction grid file import failed.";
								if (add)
									delete s;
								throw std::invalid_argument("Wind direction grid file import failed.");
							}
						}
					}
				}
		...
		if (m_sectors.size() != 8)
		{
			...
			{
				NewDirectionSector *ss = m_sectors[iii];
